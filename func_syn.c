...
extern void prt_program(A_NODE *, int);
extern void prt_initializer(A_NODE *, int);
extern void prt_arg_expr_list(A_NODE *, int);
extern void prt_statement(A_NODE *, int);
extern void prt_statement_list(A_NODE *, int);
extern void prt_for_expression(A_NODE *, int);
extern void prt_expression(A_NODE *, int);
extern void prt_A_TYPE(A_TYPE *, int);
extern void prt_A_ID_LIST(A_ID *, int);
extern void prt_A_ID(A_ID *, int);
extern void prt_A_ID_NAME(A_ID *, int);
extern void prt_STRING(char *, int);
extern void prt_integer(int, int);
extern void print_node(A_NODE *, int);
    ...
    if (t)
    {
        if (p->type)
        {
            if (p->type == t)
            {
                ...
                syntax_error(24, (char *)NIL); // fixed
        }
        else
            p->type = t;
    }
...
A_ID *setFunctionDeclaratorSpecifier(A_ID *id, A_SPECIFIER *p)
{
    A_ID *a;
    if (p->stor)
        syntax_error(25,(char *)NIL);
    setDefaultSpecifier(p);
    if (id->type->kind != T_FUNC)
    {
        syntax_error(21,(char *)NIL);
        return (id);
    }
    else
    {
    ...
    while (a)
    {
        if (strlen(a->name))
            current_id = a;
        else if (a->type && a->type != void_type) ///////////////////fixed
            syntax_error(23, (char *)NIL);
        a = a->link;
    }
    return (id);
}
...
A_ID *setParameterDeclaratorSpecifier(A_ID *id, A_SPECIFIER *p)
{
    if (searchIdentifierAtCurrentLevel(id->name, id->prev))
        syntax_error(12, id->name);
    if (p->stor || (p->type == void_type && id->name != "")) ///////fixed
        syntax_error(14,(char *)NIL);
    setDefaultSpecifier(p);
    id = setDeclaratorElementType(id, p->type);
    id->kind = ID_PARM;
    return (id);
