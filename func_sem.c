...
// void *sem_initializer(A_NODE *node) // pass

int sem_A_TYPE(A_TYPE *);

A_TYPE *sem_expression(A_NODE *); // 수식 분석
...
BOOLEAN isNotSameType(A_TYPE *, A_TYPE *); // missing
...
//statement / sw brk cnt => advanced info.
int sem_statement(A_NODE *node, int addr, A_TYPE *ret, BOOLEAN sw, BOOLEAN brk, BOOLEAN cnt)
{
    ...
    case N_STMT_WHILE:
        ...
        break;
    case N_STMT_DO:
        local_size = sem_statement(node->llink, addr, ret, sw, brk, cnt);
        t = sem_expression(node->rlink);
        if (isScalarType(t))
    ...
    case N_STMT_CONTINUE:
        if (cnt == FALSE)
            semantic_error(74, node->line, sw, brk, cnt);
        break;
    case N_STMT_BREAK:
        if (brk == FALSE)
            semantic_error(73, node->line, sw, brk, cnt);
        break;
    case N_STMT_RETURN:
        if (node->clink)
        {
...
BOOLEAN isSameParameterType(A_ID *a, A_ID *b) //fix
{
    while (a)
    {
        if (b == NIL || (a->type != b->type))  //fix
            return (FALSE);
        a = a->link;
        b = b->link;
    }
...
BOOLEAN isAllowableCastingConversion(A_TYPE *t1, A_TYPE *t2) //t1 into t2
{
    // printf("t1 = %d %d %d \n",isAnyIntegerType(t1),isFloatType(t1),isPointerType(t1));
...
A_LITERAL getTypeAndValueOfExpression(A_NODE *node)
{
    ...
    case N_EXP_IDENT:
        ...
        // else if(id->type==T_POINTER) /////////수정
        // {
        //     result.type = int_type;
        //     result.value.i = id->address;
        // }
        else
        {
            result.type = int_type;
