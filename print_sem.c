...
void prt_sem_program(A_NODE *node, int s)
{
    ...
    case N_PROGRAM:
        ...
        break;
    default:
        fprintf(yyout,"****syntax tree error******");
    }
}
...
void prt_sem_A_TYPE(A_TYPE *t, int s)
{
    ...
    if (t == int_type)
        ...
        case T_ENUM:
            ...
            fprintf(yyout,"ENUM\n");
            print_space(s);
            fprintf(yyout,"| ENUMERATORS\n");
            prt_sem_A_ID_LIST(t->field, s + 2);
            break;
        case T_POINTER:
            t->prt = FALSE;
            fprintf(yyout,"POINTER\n");
            print_space(s);
            fprintf(yyout,"| ELEMENT_TYPE\n");
...
{
    print_space(s);
    fprintf(yyout,"(ID=\"%s\") TYPE:%x KIND:%s SPEC=%s LEV=%d VAL=%d ADDR=%d\n",
